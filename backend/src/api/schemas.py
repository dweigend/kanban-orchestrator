...
# ─────────────────────────────────────────────────────────────
# Step Schemas (for Subtask Steps)
# ─────────────────────────────────────────────────────────────
...
class Step(BaseModel):
    ...
    text: str = Field(
        ...
        examples=["Read the configuration file", "Validate input parameters"],
    )
    done: bool = Field(
        default=False,
...
class SubtaskPlan(BaseModel):
    """A planned subtask generated by the AI agent.

    Used for structured output when decomposing a parent task.
    """
    ...
    title: str = Field(
        ...
        min_length=1,
        max_length=255,
        description="Brief subtask title",
        examples=["Set up database connection", "Implement authentication"],
    )
    description: str = Field(
        ...,
        description="Detailed subtask description with context",
        examples=["Create SQLite connection with proper error handling..."],
    )
    steps: list[Step] = Field(
        default_factory=list,
        description="Granular steps to complete this subtask",
    )


class TaskDecomposition(BaseModel):
    """Result of AI agent task planning.

    Contains all subtasks needed to complete a parent task.
    """

    subtasks: list[SubtaskPlan] = Field(
        ...,
        min_length=1,
        description="List of subtasks that make up the parent task",
    )
    summary: str = Field(
        ...,
        description="Brief summary of the decomposition approach",
        examples=["Breaking down into 3 subtasks: setup, implementation, testing"],
    )
