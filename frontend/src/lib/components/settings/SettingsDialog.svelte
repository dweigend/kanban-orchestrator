<script lang="ts">
import { Accordion, Dialog, Select, Separator, Slider, Switch } from 'bits-ui';
...
<Dialog.Root bind:open onOpenChange={(v) => onOpenChange?.(v)}>
	<Dialog.Portal>
		...
		/>
		<Dialog.Content
			class="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 border border-[var(--border-default)] bg-[var(--bg-elevated)] rounded-lg shadow-xl data-[state=open]:animate-fade-in"
		>
			<!-- Header -->
			<div class="flex items-center justify-between p-4 border-b border-[var(--border-muted)]">
				...
				<div class="flex items-center gap-2">
					<button
						...
						class="px-3 py-1.5 text-xs border border-[var(--border-default)] rounded hover:bg-[var(--bg-hover)] transition-colors"
					>
						DISCARD
					</button>
					<button
			...
			<!-- Content -->
			<div class="p-4 max-h-[70vh] overflow-y-auto scrollbar-thin">
				<Accordion.Root type="multiple" value={['editor']}>
					<!-- Appearance -->
					<Accordion.Item
						value="appearance"
						class="border border-[var(--border-muted)] rounded mb-2"
					>
						<Accordion.Header>
							<Accordion.Trigger
			...
			<!-- Close Button -->
			<Dialog.Close
				class="absolute right-4 top-4 p-1 rounded hover:bg-[var(--bg-hover)] transition-colors"
			>
				<X class="size-5 text-[var(--text-muted)]" />
